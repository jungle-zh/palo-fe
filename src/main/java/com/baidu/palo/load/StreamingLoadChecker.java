...
public class StreamingLoadChecker extends Daemon {
    private static final Logger LOG = LogManager.getLogger(StreamingLoadChecker.class);

    // checkers for running job state
    private static Map<JobState, StreamingLoadChecker> checkers = Maps.newHashMap();
    ...
    private StreamingLoadChecker(JobState jobState, long intervalMs) {
    ...
    public static void init(long intervalMs) {
        checkers.put(JobState.PENDING, new StreamingLoadChecker(JobState.PENDING, intervalMs));
        checkers.put(JobState.ETL, new StreamingLoadChecker(JobState.ETL, intervalMs));
        checkers.put(JobState.LOADING, new StreamingLoadChecker(JobState.LOADING, intervalMs));
        checkers.put(JobState.QUORUM_FINISHED, new StreamingLoadChecker(JobState.QUORUM_FINISHED, intervalMs));
    ...
    public static void startAll() {
        for (StreamingLoadChecker loadChecker : checkers.values()) {
            loadChecker.start();
        }
    }
